...
{
    // Create nodeA
    UdcServer* nodeA = udcCreateServer(0x01020304, 1234, 2345, "test_reconnect_ipv4_logA.txt");
    ...
    // Create nodeB
    UdcServer* nodeB = udcCreateServer(0x01020304, 1235, 2346, "test_reconnect_ipv4_logB.txt");
    ...
    while (!connected)
    {
        ...
        {
            auto eventType = udcGetEventType(event);
            ...
            default:
                udcDeleteServer(nodeA);
                udcDeleteServer(nodeB);
                std::cout << "received event other than connection success: " << eventType << "\n";
                return -1;
            }
        }
        ...
        while ((event = udcProcessEvents(nodeA)) != nullptr)
        {
            ...
            switch(eventType)
            {
                ...
                default:
                    udcDeleteServer(nodeA);
                    udcDeleteServer(nodeB);
                    std::cout << "received event other than connection lost" << eventType << "\n";
                    return -1;
            }
        }

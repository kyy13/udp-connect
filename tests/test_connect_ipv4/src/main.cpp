...
int main()
{
    ...
    // Create nodeA
    UdcServer* nodeA = udcCreateServer(sig, 1234, 2345, buffer.data(), buffer.size(), "test_connect_ipv4_logA.txt");
    ...
    // Create nodeB
    UdcServer* nodeB = udcCreateServer(sig, 1235, 2346, buffer.data(), buffer.size(), "test_connect_ipv4_logB.txt");
